...
d3.csv("books.csv", function(csv) {
  ...
  // nesting based on genre and year
  var nestedData = d3
    .nest()
    .key(function(d) {
      return d.Genre;
  ...
  function generateGraphs() {
    ...
    var fictionYearMap = []; // array of values for non fiction books, ie 2009 : 20
    var nonFictionYearMap = []; // array of values for fiction books, ie 2009 : 23
    var meanPricesMap = [];
    var nonFictionKeys = Object.keys(nestedData["Non Fiction"]);

    //max and min for yaxis and mapping number of non-fiction books to years
    for (var i = 0; i < nonFictionKeys.length; i++) {
      var currKey = nonFictionKeys[i];
      var val = nestedData["Non Fiction"][currKey].length;
      if (val < min) {
        min = val;
      }
      if (val > max) {
      ...
      nonFictionYearMap.push({
        x: currKey,
        y: val
      });
    }

    var fictionKeys = Object.keys(nestedData["Fiction"]);

    //max and min for yaxis and mapping number of fiction books to years
    for (var i = 0; i < fictionKeys.length; i++) {
      var currKey = fictionKeys[i];
      var val = nestedData["Fiction"][currKey].length;
      if (val < min) {
        min = val;
      }
      if (val > max) {
    ...
    //map year to mean prices of books 
    ...
    //adding fiction line to line chart
    chart.append("path")
      .attr("class", "line")
      .transition()
      .attr("d", fictionLine(fictionYearMap))
      .attr("transform", "translate(55, 0)")
      .style('stroke', 'red')
      .style('fill', 'none');
    ...
    // adding non fiction line to line chart
    chart.append("path")
      .attr("class", "line")
      .transition()
      .attr("d", nonFictionLine(nonFictionYearMap))
      .attr("transform", "translate(55, 0)")
      .style('stroke', 'blue')
      .style('fill', 'none');
    ...
    var fiction_circles = chart
      ...
      .on("mouseover", function(d, i) {
        // hover event to display top 5 rated books
        year = d.x
        numberOfBooks = d.y
        ficYearBooks = nestedData["Fiction"][d.x]
    ...
    // adding animation for fiction circles
    fiction_circles
      .attr("cx", function(d) {
    ...
    var non_fiction_circles = chart
      ...
      .on("mouseover", function(d, i) { // hover event to display top 5 rated books
        ...
        year = d.x
        numberOfBooks = d.y
        nonficYearBooks = nestedData["Non Fiction"][d.x]
  ...
  // filter on clicking button "Filter Data" for line chart
  d3.select(filter1)
    .append('p')
    .append('button')
  ...
  // filter on clicking button "Reset Filter" for line chart
  d3.select(filter1)
    .append('p')
    .append('button')
    .style("border", "1px solid black")
  ...
  // filter on clicking button "Filter Data" for bar chart
  d3.select(filter2)
    .append('p')
    .append('button')
    .style('border', '1px solid black')
  ...
  // filter on clicking button "Reset Data" for bar chart
  d3.select(filter2)
    .append('p')
    .append('button')
    .style("border", "1px solid black")
